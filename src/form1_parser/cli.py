...
from .scheduler.stage5 import create_stage5_scheduler
from .scheduler.stage6 import create_stage6_scheduler
from .scheduler.stage7 import create_stage7_scheduler
...
DEFAULT_INSTRUCTOR_AVAILABILITY_JSON = Path(
    "data/reference/instructor-availability.json"
)
DEFAULT_NEARBY_BUILDINGS_JSON = Path("data/reference/nearby-buildings.json")
DEFAULT_INSTRUCTOR_DAYS_JSON = Path("data/reference/instructor-days.json")
DEFAULT_GROUPS_SECOND_SHIFT_CSV = Path("data/reference/groups-second-shift.csv")
...
def schedule(
    ...
    if not streams:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No streams found in input file"
        )
        raise typer.Exit(1)
    ...
    # Show scheduling success rate
    stats = result.statistics
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Stage 1 Scheduling Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} hours "
    ...
    if verbose and result.unscheduled_stream_ids:
        ...
        if len(result.unscheduled_stream_ids) > 10:
            console.print(
                f"  [yellow]... and {len(result.unscheduled_stream_ids) - 10} more[/yellow]"
            )
...
def generate_excel(
    ...
    if language and language not in ("kaz", "rus"):
        console.print(
            f"[bold red]Error:[/bold red] Invalid language: {language}. Use 'kaz' or 'rus'."
        )
        raise typer.Exit(1)
    ...
    if year and year not in (1, 2, 3, 4):
        console.print(
            f"[bold red]Error:[/bold red] Invalid year: {year}. Use 1, 2, 3, or 4."
        )
        raise typer.Exit(1)
    ...
    if week_type and week_type not in ("odd", "even"):
        console.print(
            f"[bold red]Error:[/bold red] Invalid week type: {week_type}. Use 'odd' or 'even'."
        )
        raise typer.Exit(1)
    ...
    if not generated_files:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No files generated. No groups matched the criteria."
        )
        raise typer.Exit(1)
...
@app.command("generate-instructor-excel")
def generate_instructor_excel(
    input_file: Annotated[
        Path,
        typer.Argument(help="Schedule JSON file"),
    ],
    ...
    """Generate Excel schedule files organized by instructor."""
    if not input_file.exists():
        console.print(f"[bold red]Error:[/bold red] File not found: {input_file}")
        raise typer.Exit(1)
    ...
    # Default output directory
    output_path = output_dir or Path("output/instructors")

    console.print(
        f"\n[bold]Generating instructor schedules from:[/bold] {input_file.name}"
    )
    if language:
        console.print(f"  Language: {language}")

    with console.status("[bold green]Generating Excel files..."):
        generated_files = generate_instructor_schedule_excel(
            input_path=input_file,
            output_dir=output_path,
            language=language,
        )
    ...
    if not generated_files:
        console.print("[bold yellow]Warning:[/bold yellow] No files generated.")
        raise typer.Exit(1)
...
@app.command("schedule-stage2")
def schedule_stage2(
    parsed_file: Annotated[
        Path,
        typer.Argument(help="Parsed JSON file from form1-parser parse command"),
    ],
    stage1_file: Annotated[
        Path,
        typer.Argument(
            help="Stage 1 schedule JSON file from form1-parser schedule command"
        ),
    ],
    output: Annotated[
        Optional[Path],
    ...
    """Generate Stage 2 schedule for multi-group practicals."""
    ...
    if not stage1_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Stage 1 file not found: {stage1_file}"
        )
        raise typer.Exit(1)
    ...
    with console.status("[bold green]Loading data..."):
        ...
        stage1_data = load_parsed_data(stage1_file)
    ...
    stage1_assignments = stage1_data.get("assignments", [])
    if not stage1_assignments:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No assignments found in Stage 1 file"
        )
        raise typer.Exit(1)

    console.print(f"\n[bold]Stage 2 Schedule Generation[/bold]")
    console.print(f"  Parsed file: {parsed_file.name}")
    console.print(f"  Stage 1 file: {stage1_file.name}")
    console.print(f"  Total streams in input: {len(streams)}")
    console.print(f"  Stage 1 assignments: {len(stage1_assignments)}")

    with console.status("[bold green]Creating Stage 2 schedule..."):
        scheduler = create_stage2_scheduler(
            rooms_path,
            subject_rooms_path if subject_rooms_path.exists() else None,
            instructor_rooms_path if instructor_rooms_path.exists() else None,
            group_buildings_path if group_buildings_path.exists() else None,
        ...
        result = scheduler.schedule(streams, stage1_assignments)

    # Count Stage 2 specific assignments
    stage2_count = result.total_assigned - len(stage1_assignments)

    # Show summary
    console.print("\n[bold]Stage 2 Schedule Results:[/bold]")
    console.print(f"  Stage 1 assignments (carried over): {len(stage1_assignments)}")
    console.print(f"  Stage 2 assignments (new): {stage2_count}")
    console.print(f"  Total assignments: {result.total_assigned}")
    console.print(f"  Unscheduled practicals: {result.total_unscheduled}")

    # Show scheduling success rate
    stats = result.statistics
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Stage 2 Scheduling Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} hours "
            f"([green]{stats.success_rate:.1f}%[/green])"
        )
    ...
    if verbose and result.unscheduled_streams:
        console.print(
            f"\n[bold yellow]Unscheduled practicals ({len(result.unscheduled_streams)}):[/bold yellow]"
        )
    ...
    if output:
        ...
        output_path = Path("output/schedule-s2.json")
...
@app.command("schedule-stage3")
def schedule_stage3(
    parsed_file: Annotated[
        Path,
        typer.Argument(help="Parsed JSON file from form1-parser parse command"),
    ],
    stage2_file: Annotated[
        Path,
        typer.Argument(
            help="Stage 2 schedule JSON file from form1-parser schedule-stage2 command"
        ),
    ],
    output: Annotated[
        Optional[Path],
    ...
    """Generate Stage 3 schedule for practical-only streams (no lecture dependency)."""
    if not parsed_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Parsed file not found: {parsed_file}"
        )
    ...
    if not stage2_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Stage 2 file not found: {stage2_file}"
        )
        raise typer.Exit(1)
    ...
    with console.status("[bold green]Loading data..."):
        data = load_parsed_data(parsed_file)
        stage2_data = load_parsed_data(stage2_file)
    ...
    stage2_assignments = stage2_data.get("assignments", [])
    if not stage2_assignments:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No assignments found in Stage 2 file"
        )
        raise typer.Exit(1)

    stage2_unscheduled = stage2_data.get("unscheduled_streams", [])

    console.print(f"\n[bold]Stage 3 Schedule Generation[/bold]")
    console.print(f"  Parsed file: {parsed_file.name}")
    console.print(f"  Stage 2 file: {stage2_file.name}")
    console.print(f"  Total streams in input: {len(streams)}")
    console.print(f"  Stage 2 assignments: {len(stage2_assignments)}")

    with console.status("[bold green]Creating Stage 3 schedule..."):
        scheduler = create_stage3_scheduler(
            rooms_path,
            subject_rooms_path if subject_rooms_path.exists() else None,
            instructor_rooms_path if instructor_rooms_path.exists() else None,
            group_buildings_path if group_buildings_path.exists() else None,
        ...
        result = scheduler.schedule(streams, stage2_assignments, stage2_unscheduled)

    # Count Stage 3 specific assignments
    stage3_count = result.total_assigned - len(stage2_assignments)

    # Show summary
    console.print("\n[bold]Stage 3 Schedule Results:[/bold]")
    console.print(f"  Previous assignments (carried over): {len(stage2_assignments)}")
    console.print(f"  Stage 3 assignments (new): {stage3_count}")
    console.print(f"  Total assignments: {result.total_assigned}")
    console.print(f"  Unscheduled practicals: {result.total_unscheduled}")
    ...
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Stage 3 Scheduling Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} hours "
            f"([green]{stats.success_rate:.1f}%[/green])"
        )
...
def schedule_stage4(
    ...
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Stage 4 Scheduling Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} hours "
            f"([green]{stats.success_rate:.1f}%[/green])"
        )
...
@app.command("schedule-stage5")
def schedule_stage5(
    parsed_file: Annotated[
        Path,
        typer.Argument(help="Parsed JSON file from form1-parser parse command"),
    ],
    stage4_file: Annotated[
        Path,
        typer.Argument(
            help="Stage 4 schedule JSON file from form1-parser schedule-stage4 command"
        ),
    ],
    output: Annotated[
        Optional[Path],
    ...
    """Generate Stage 5 schedule for single-group practicals with lecture dependency."""
    if not parsed_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Parsed file not found: {parsed_file}"
        )
    ...
    if not stage4_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Stage 4 file not found: {stage4_file}"
        )
        raise typer.Exit(1)
    ...
    with console.status("[bold green]Loading data..."):
        data = load_parsed_data(parsed_file)
        stage4_data = load_parsed_data(stage4_file)
    ...
    stage4_assignments = stage4_data.get("assignments", [])
    if not stage4_assignments:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No assignments found in Stage 4 file"
        )
        raise typer.Exit(1)

    stage4_unscheduled = stage4_data.get("unscheduled_streams", [])

    console.print(f"\n[bold]Stage 5 Schedule Generation[/bold]")
    console.print(f"  Parsed file: {parsed_file.name}")
    console.print(f"  Stage 4 file: {stage4_file.name}")
    console.print(f"  Total streams in input: {len(streams)}")
    console.print(f"  Stage 4 assignments: {len(stage4_assignments)}")

    with console.status("[bold green]Creating Stage 5 schedule..."):
        scheduler = create_stage5_scheduler(
            rooms_path,
            subject_rooms_path if subject_rooms_path.exists() else None,
            instructor_rooms_path if instructor_rooms_path.exists() else None,
            group_buildings_path if group_buildings_path.exists() else None,
        ...
        result = scheduler.schedule(streams, stage4_assignments, stage4_unscheduled)

    # Count Stage 5 specific assignments
    stage5_count = result.total_assigned - len(stage4_assignments)

    # Show summary
    console.print("\n[bold]Stage 5 Schedule Results:[/bold]")
    console.print(f"  Previous assignments (carried over): {len(stage4_assignments)}")
    console.print(f"  Stage 5 assignments (new): {stage5_count}")
    console.print(f"  Total assignments: {result.total_assigned}")
    console.print(f"  Unscheduled practicals: {result.total_unscheduled}")
    ...
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Stage 5 Scheduling Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} hours "
            f"([green]{stats.success_rate:.1f}%[/green])"
        )
    ...
    if output:
        ...
        output_path = Path("output/schedule-s5.json")
...
@app.command("schedule-stage6")
def schedule_stage6(
    parsed_file: Annotated[
        Path,
        typer.Argument(help="Parsed JSON file from form1-parser parse command"),
    ],
    stage5_file: Annotated[
        Path,
        typer.Argument(
            help="Stage 5 schedule JSON file from form1-parser schedule-stage5 command"
        ),
    ],
    output: Annotated[
        Optional[Path],
    ...
    """Generate Stage 6 schedule for all lab streams."""
    if not parsed_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Parsed file not found: {parsed_file}"
        )
    ...
    if not stage5_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Stage 5 file not found: {stage5_file}"
        )
        raise typer.Exit(1)
    ...
    with console.status("[bold green]Loading data..."):
        data = load_parsed_data(parsed_file)
        stage5_data = load_parsed_data(stage5_file)
    ...
    stage5_assignments = stage5_data.get("assignments", [])
    if not stage5_assignments:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No assignments found in Stage 5 file"
        )
        raise typer.Exit(1)

    stage5_unscheduled = stage5_data.get("unscheduled_streams", [])

    console.print(f"\n[bold]Stage 6 Schedule Generation[/bold]")
    console.print(f"  Parsed file: {parsed_file.name}")
    console.print(f"  Stage 5 file: {stage5_file.name}")
    console.print(f"  Total streams in input: {len(streams)}")
    console.print(f"  Stage 5 assignments: {len(stage5_assignments)}")

    with console.status("[bold green]Creating Stage 6 schedule..."):
        scheduler = create_stage6_scheduler(
            rooms_path,
            subject_rooms_path if subject_rooms_path.exists() else None,
            instructor_rooms_path if instructor_rooms_path.exists() else None,
            group_buildings_path if group_buildings_path.exists() else None,
        ...
        result = scheduler.schedule(streams, stage5_assignments, stage5_unscheduled)

    # Count Stage 6 specific assignments
    stage6_count = result.total_assigned - len(stage5_assignments)

    # Show summary
    console.print("\n[bold]Stage 6 Schedule Results:[/bold]")
    console.print(f"  Previous assignments (carried over): {len(stage5_assignments)}")
    console.print(f"  Stage 6 assignments (new): {stage6_count}")
    console.print(f"  Total assignments: {result.total_assigned}")
    console.print(f"  Unscheduled labs: {result.total_unscheduled}")

    # Show scheduling success rate
    stats = result.statistics
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Stage 6 Scheduling Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} hours "
            f"([green]{stats.success_rate:.1f}%[/green])"
        )
    ...
    if verbose and result.unscheduled_streams:
        console.print(
            f"\n[bold yellow]Unscheduled labs ({len(result.unscheduled_streams)}):[/bold yellow]"
        )
        for stream in result.unscheduled_streams[:10]:
            console.print(
                f"  [yellow]- {stream.stream_id}: {stream.reason.value}[/yellow]"
    ...
    if output:
        ...
        output_path = Path("output/schedule-s6.json")
...
@app.command("schedule-stage7")
def schedule_stage7(
    parsed_file: Annotated[
        Path,
        typer.Argument(help="Parsed JSON file from form1-parser parse command"),
    ],
    stage6_file: Annotated[
        Path,
        typer.Argument(
            help="Stage 6 schedule JSON file from form1-parser schedule-stage6 command"
        ),
    ],
    output: Annotated[
        Optional[Path],
    ...
    """Optimize schedule: fill empty days, reschedule unscheduled streams."""
    if not parsed_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Parsed file not found: {parsed_file}"
        )
    ...
    if not stage6_file.exists():
        console.print(
            f"[bold red]Error:[/bold red] Stage 6 file not found: {stage6_file}"
        )
        raise typer.Exit(1)
    ...
    with console.status("[bold green]Loading data..."):
        data = load_parsed_data(parsed_file)
        stage6_data = load_parsed_data(stage6_file)
    ...
    stage6_assignments = stage6_data.get("assignments", [])
    if not stage6_assignments:
        console.print(
            "[bold yellow]Warning:[/bold yellow] No assignments found in Stage 6 file"
        )
        raise typer.Exit(1)

    stage6_unscheduled = stage6_data.get("unscheduled_streams", [])

    console.print(f"\n[bold]Stage 7 Schedule Optimization[/bold]")
    console.print(f"  Parsed file: {parsed_file.name}")
    console.print(f"  Stage 6 file: {stage6_file.name}")
    console.print(f"  Total streams in input: {len(streams)}")
    console.print(f"  Stage 6 assignments: {len(stage6_assignments)}")
    console.print(f"  Stage 6 unscheduled: {len(stage6_unscheduled)}")

    with console.status("[bold green]Running Stage 7 optimization..."):
        scheduler = create_stage7_scheduler(
            rooms_path,
            subject_rooms_path if subject_rooms_path.exists() else None,
            instructor_rooms_path if instructor_rooms_path.exists() else None,
            group_buildings_path if group_buildings_path.exists() else None,
        ...
        result = scheduler.schedule(streams, stage6_assignments, stage6_unscheduled)

    # Show summary
    console.print("\n[bold]Stage 7 Optimization Results:[/bold]")
    console.print(f"  Total assignments: {result.total_assigned}")
    console.print(f"  Remaining unscheduled: {result.total_unscheduled}")

    # Show improvement
    improvement = len(stage6_unscheduled) - result.total_unscheduled
    if improvement > 0:
        console.print(
            f"\n[bold green]Improvement:[/bold green] "
            f"Scheduled {improvement} previously unscheduled streams"
        )
    ...
    if stats.expected_hours > 0:
        console.print(
            f"\n[bold]Phase 7B Success Rate:[/bold] "
            f"{stats.scheduled_hours}/{stats.expected_hours} streams "
            f"([green]{stats.success_rate:.1f}%[/green])"
        )
    ...
    if verbose and result.unscheduled_streams:
        console.print(
            f"\n[bold yellow]Remaining unscheduled ({len(result.unscheduled_streams)}):[/bold yellow]"
        )
        # Group by reason
        by_reason: dict[str, int] = {}
        for stream in result.unscheduled_streams:
            reason = stream.reason.value
            by_reason[reason] = by_reason.get(reason, 0) + 1
        for reason, count in sorted(by_reason.items(), key=lambda x: -x[1]):
            console.print(f"  [yellow]{reason}: {count}[/yellow]")
    ...
    if output:
        ...
        output_path = Path("output/schedule-s7.json")
...
@app.command("generate-unscheduled-excel")
def generate_unscheduled_excel_cmd(
    input_file: Annotated[
        Path,
        typer.Argument(help="Schedule JSON file"),
    ],
    ...
    """Generate Excel report of unscheduled streams (in Russian)."""
    if not input_file.exists():
        console.print(f"[bold red]Error:[/bold red] File not found: {input_file}")
        raise typer.Exit(1)

    # Default output path
    output_path = output or Path("output/unscheduled_streams.xlsx")

    console.print(
        f"\n[bold]Generating unscheduled streams report from:[/bold] {input_file.name}"
    )

    with console.status("[bold green]Generating Excel report..."):
        result_path = generate_unscheduled_excel(
            input_path=input_file,
            output_path=output_path,
        )

    console.print(
        f"\n[bold green]✓[/bold green] Report generated: {result_path}"
    )
